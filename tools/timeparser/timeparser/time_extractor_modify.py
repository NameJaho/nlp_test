...
try:
    from .rule_pattern import TIME_CHAR_STRING, \
        FAKE_POSITIVE_START_STRING, FAKE_POSITIVE_END_STRING, TIME_CHAR_STRING_FAST
...
def timer(func):
    def wrapper(*args, **kwargs):
        ...
        print(f"{class_name}.{func.__name__} executed in {execution_time:.4f} seconds.")
        return result
...
class TimeExtractor(object):
    ...
    @timer
    def _prepare(self, fast):
        self.parse_time = TimeParser()
        rule = TIME_CHAR_STRING
        if fast:
            rule = TIME_CHAR_STRING_FAST
        self.time_string_pattern = re.compile(rule)  # 该正则存在假阴风险
    ...
    def __call__(self, text, time_base=time.time(), with_parsing=True, ret_all=False,
                 ret_type='str', ret_future=False, period_results_num=None, fast=True):
        if self.parse_time is None:
            self._prepare(fast=fast)

        candidates_list = self.extract_time_candidates(text)
        print(len(candidates_list))
        print([i['time_candidate'] for i in candidates_list])
        ...
        for candidate in candidates_list:
            offset = [0, 0]
            bias = 0
            if self.dup_cand_value.get(candidate['time_candidate']):
                for item in self.dup_cand_value[candidate['time_candidate']]:
                    if not item:
                        continue
                    true_string, result, offset = item
                    print(f"dup: {candidate['time_candidate']} 已经跑过了 {true_string}")
                    if true_string is not None:
                        ...
                        if with_parsing:
                            time_entity_list.append(
                                {'text': true_string,
                                 'offset': [candidate['offset'][0] + offset[0],
                                            candidate['offset'][0] + offset[1]],
                                 'type': result['type'],
                                 'detail': result})
                        else:
                            time_entity_list.append(
                ...
                self.dup_cand_value[candidate['time_candidate']] = []
                while candidate['offset'][0] + offset[1] < candidate['offset'][1]:
                    ...
                    self.dup_cand_value[candidate['time_candidate']].append([true_string, result, offset])
                    grip += time.time() - s
    ...
    def extract_time_candidates(self, text):
        ...
        dup = list()
        pattern = r'\b(至\d+|以上|以内|以下|——|本上)\b'
        p = re.compile(pattern)
        while idx_count < text_length:
            matched_res = self.time_string_pattern.search(text[idx_count:])
            if matched_res is not None:
                if len(matched_res.group()) > 1 and matched_res.group():  # and matched_res.group() not in dup
                    dup.append(matched_res.group())
                    # 1 去除小于2000的数字 以及大于2100的数字
                    if matched_res.group().isdigit():
                        if int(matched_res.group()) > 2000 and int(matched_res.group()) < 2099:
                            # 避免 14001:2015
                            if text[idx_count + matched_res.span()[0] - 1] != ':':
                        ...
                        # 从字符串中找出数字 并且判断数字是否大于 2099
                        num = re.findall(r'\d+', matched_res.group())
                        flag = False
                        if num:
                            for item in num:
                                # 判断改情况：2020年1600
                                if '年' in matched_res.group():
                                    continue
                                if int(item) > 2099:
                                    flag = True
                                elif 1000 < int(item) < 2000:
                                    flag = True
                        if not flag:  # 任一数字大于2099 则去掉 或 1000-2000
                            time_candidates_list.append(
                                {'time_candidate': matched_res.group(),
                                 'offset': [idx_count + matched_res.span()[0],
                                            idx_count + matched_res.span()[1]]})
                        ...
                        match = p.search(matched_res.group())
                        # print(matched_res.group())
                        if match:
                            # print(f"Matched: '{match.group()}' in '{matched_res.group()}'")
                            idx_count += matched_res.span()[1]

                            continue
                        else:
                            # print(f"No match in '{matched_res.group()}'")
                            pass
...
if __name__ == '__main__':
    text = '''
    2023年asdfc做了什么 2021Q1 做了什么 2022第一季度干嘛，
            '''
